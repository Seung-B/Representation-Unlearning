...
def t_sne_results(features, labels, tsne):
    model = TSNE(n_components=2)
    features = np.array([ft.numpy() for ft in features])
    features = features.reshape(len(features), 1024)
    res = model.fit_transform(np.array(features))

    indices = [i for i, item in enumerate(labels) if int(item) in tsne]
    print(indices)

    ul_data = features[indices]

    plt.scatter(res[::10, 0], res[::10, 1], color='blue', alpha = 0.01)
    plt.scatter(ul_data[:, 0], ul_data[:, 1], color='red', marker='x')
    plt.savefig("./t-sne/t-sne_fig_coco.png")
    print("T-SNE Figure Generated in ./t-sne.")

    return None

def evaluate(model, eval_dataloader, dataset_classes, WordnetId2ClassName, tokenizer, device, dataset_type, tsne):
    top_1_correct = 0
    top_5_correct = 0
    ...
    with torch.no_grad():
        ...
        for step, (images, labels) in enumerate(tqdm(eval_dataloader)):
            ...
            # collect image_features for t_sne
            if tsne:
                img_features_list.append(image_features.cpu())
        ...
        if tsne:
            t_sne_results(img_features_list, labels_list, tsne)
...
def main():
    ...
    parser.add_argument("--tsne", default=None, nargs="+", type=int, help="T-SNE graph results")

    args = parser.parse_args()

    zero_shot_eval_output_dir = "zero_shot_eval_output"
    t_sne_eval_output_dir = "t-sne"
    # creating directory to store demo result
    mkdir(path=zero_shot_eval_output_dir)
    mkdir(path=t_sne_eval_output_dir)
    ...
    if args.dataset_type == 'imagenet':
        ...
        imageDataset = torchvision.datasets.ImageFolder(root=args.data_dir + 'val', transform = transform)
    ...
    # now evaluate
    top_1_accuracy, top_5_accuracy, class_wise_top_1_accuracy, class_wise_top_5_accuracy = evaluate(model, eval_dataloader, dataset_classes, WordnetId2ClassName, tokenizer, device, args.dataset_type, args.tsne)
